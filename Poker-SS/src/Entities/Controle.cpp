	...
	void Controle::OnImGuiRender()
	{
		...
		else
		{
			if (m_AdicionandoFichas)
			{
				...
				ImGui::DragInt("+", &m_FichasAdicionar, 200, 0, 5000, "$ %d");
				...
				if (ImGui::Button("Cancelar", ImVec2(80, 30)))
				{
					m_AdicionandoFichas = false;
					m_FichasAdicionar = 0;
				}
			...
			{
				ImGui::Begin("Remover Fichas");
				ImGui::Text("%s - %d", m_EngineJogo->GetJogadores()[m_SelectedPlayer]->GetName().c_str(), m_EngineJogo->GetJogadores()[m_SelectedPlayer]->GetChips());
				ImGui::DragInt("-", &m_FichasAdicionar, 200, 0, m_EngineJogo->GetJogadores()[m_SelectedPlayer]->GetChips(), "$ %d");
				if (ImGui::Button("OK", ImVec2(80, 30)))
				{
			...
			if (m_AdicionandoJogador)
			{
				...
				if (ImGui::Button("OK", ImVec2(80, 30)) || ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Enter)))
				{
					if (strlen(m_NomeJogadorInclusao))
					{
				...
				if (ImGui::Button("Cancelar", ImVec2(80, 30)) || ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Escape)))
				{
			...
			case TexasHoldem::EstadoJogo::PreJogo:
				...
				ImGui::ListBoxFooter();
				if (ImGui::Button("Adicionar Jogador", ImVec2(130, 30)))
				{
					m_PrimeiroFoco = true;
				...
				{
					if (ImGui::Button("Adicionar Fichas", ImVec2(130, 30)))
					{
				...
				{
					if (ImGui::Button("Remover Fichas", ImVec2(130, 30)))
					{
				...
				{
					if (ImGui::Button("INICIAR!", ImVec2(130, 30)))
					{
						m_EngineJogo->Start();
					}
				}
