...
Pratica::Pratica(uint32_t width, uint32_t height, uint32_t pixelSize)
	: m_Width(width), 
	m_Height(height), 
...
void Pratica::OnUpdate(SolutionShelves::Timestep ts)
{

}

void Pratica::OnEvent(SolutionShelves::Event& event)
{

}

void Pratica::OnRender(SolutionShelves::OrthographicCameraController& cameraController)
{
...
void Pratica::DrawPixel(uint32_t x, uint32_t y, const glm::vec4& color)
{
	uint32_t posIndex = y * m_ScreenWidth + x;
...
void Pratica::ClearScreen(const glm::vec4& color)
{
	for (uint32_t y = 0; y < m_ScreenHeight; y++)
	{
		for (uint32_t x = 0; x < m_ScreenWidth; x++)
...
void Pratica::Noise()
{
	for (uint32_t y = 0; y < m_ScreenHeight; y++)
	{
		for (uint32_t x = 0; x < m_ScreenWidth; x++)
		{
			uint32_t posIndex = y * m_ScreenWidth + x;
			Pixel& pixel = m_ScreenBuffer[posIndex];
			pixel.Color = { RandomFloat(), RandomFloat(), RandomFloat(), 1.0f };
		}
	}
}
